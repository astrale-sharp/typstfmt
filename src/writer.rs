...
impl Writer {
    ...
    ///     w.push("hello");
    /// });
    /// similar_asserts::assert_eq!(writer.buffer(), Some("hello"));
    /// writer.empty_buffer();
    /// similar_asserts::assert_eq!(writer.buffer(), None);
    /// ```
    ...
    ///     w.push("world");
    /// });
    /// similar_asserts::assert_eq!(writer.buffer(), Some("world"));
    /// similar_asserts::assert_eq!(writer.result(), "hello, ");
    /// writer.flush();
    /// similar_asserts::assert_eq!(writer.buffer(), None);
    /// similar_asserts::assert_eq!(writer.result(),"hello, world");
    /// ```
    ...
    /// ```
    /// let mut writer = Writer::default();
    /// similar_asserts::assert_eq!(writer.result(), "");
    /// similar_asserts::assert_eq!(writer.buffer(), None);
    /// writer.buffered(false, |w| {
    ///     w.push("hello,")
    ///         .newline()
    ...
    /// });
    /// similar_asserts::assert_eq!(writer.result(), "");
    /// similar_asserts::assert_eq!(writer.buffer(), "hello,\nworld");
    /// ```
    ...
    /// similar_asserts::assert_eq!(ctx.buffer(), "");
    /// ctx.indent();
    ...
    /// similar_asserts::assert_eq!(ctx.buffer, "Hello, world!");
    /// ```
    ...
    /// ```
    /// let mut ctx = Context::default();
    /// similar_asserts::assert_eq!(ctx.indent_level(), 4);
    /// ctx.update_indent(|i| i * 2);
    /// similar_asserts::assert_eq!(ctx.indent_level(), 8);
    /// ````
    ...
    /// ctx.newline_with_indent();
    /// ctx.push(")")
    /// similar_asserts::assert_eq!(ctx.buffer(), "f(\n    a, b\n)");
    /// ```
...
mod tests {
    ...
    fn test_buffered() {
        ...
        writer.buffered(false, |w| {
            w.inc_indent().newline_with_indent().push("x + y");
        });
        similar_asserts::assert_eq!(
            writer.buffer,
            Some(format!("\n{}x + y", " ".repeat(indent)))
        );
        similar_asserts::assert_eq!(writer.result, "let add(x, y) =");
        writer.flush();
        similar_asserts::assert_eq!(
            writer.result,
            format!("let add(x, y) =\n{}x + y", " ".repeat(indent))
